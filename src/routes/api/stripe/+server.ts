...
export const POST: RequestHandler = async ({ request, platform, url }) => {
	...
	if (!valid) return json({ ok: false, error: 'invalid signature' }, { status: 400 });
	...
	await env.STRIPE_EVENTS.put(dedupKey, '1', { expirationTtl: STRIPE_EVENT_TTL_SECONDS });

	const session = event.data.object as Stripe.Checkout.Session;
	const dropId = session.metadata?.drop_id;
	if (!dropId) {
		await logAttempt(env.DB, { prompt: '(webhook)', status: 'storage_error', error: 'missing drop_id metadata' });
	...
	try {
		...
		await logAttempt(env.DB, { prompt: '(webhook)', status: 'storage_error', error: err instanceof Error ? err.message : String(err), dropId });
	}

	// Discord notification.
	const customerEmail = session.customer_details?.email ?? session.customer_email ?? 'unknown';
	const shippingName = session.collected_information?.shipping_details?.name ?? session.customer_details?.name ?? 'unknown';
	const shippingAddress = formatAddress(session.collected_information?.shipping_details?.address ?? session.customer_details?.address ?? null);
	...
	const drop = await env.DB.prepare(`SELECT prompt, mockup_url FROM drops WHERE id = ?`)
		.bind(dropId)
