<script lang="ts">
	...
	const formattedTimestamp = $derived(
		new Date(data.drop.created_at).toISOString().replace('T', ' ').replace(/\..+/, ' UTC')
	);
</script>
...
<div class="container">
	...
	<div class="drop-card">
		...
		<div class="mockup">
			<img src={data.drop.mockup_url} alt="shirt mockup" loading="eager" decoding="async" />
		</div>
		...
		<div class="stats">
			<div class="stat"><div class="v">{data.drop.sold_count}</div><div class="k">SOLD</div></div>
			<div class="stat"><div class="v">{data.drop.view_count}</div><div class="k">VIEWED</div></div>
			<div class="stat">
...
<style>
	...
	.drop-card {
		...
		border: 1px solid #25252b; border-radius: 4px;
		padding: 0 0 20px; overflow: hidden;
	}
	.drop-meta {
		display: flex; justify-content: space-between; align-items: center;
		padding: 14px 18px; border-bottom: 1px solid #25252b;
		font-family: 'JetBrains Mono', ui-monospace, monospace;
		font-size: 11px; color: #8a8a93;
	}
	.drop-meta .id { color: #fff; }
	.mockup {
		width: 100%; aspect-ratio: 1/1;
		background: radial-gradient(ellipse at center, #1a1a22 0%, #0a0a0c 70%);
		display: flex; align-items: center; justify-content: center;
	}
	.mockup img { width: 100%; height: 100%; object-fit: contain; }
	.quote {
		margin: 18px 18px 0;
		padding: 12px 14px;
		background: #0a0a0c; border-left: 2px solid #00ff88;
		font-size: 14px; line-height: 1.5; color: #c5c5cc;
		font-style: italic;
	}
	.ticker {
	...
	.ticker .label {
		font-family: 'JetBrains Mono', ui-monospace, monospace;
		font-size: 10px; color: #8a8a93; letter-spacing: 0.1em;
	}
	.ticker .price { font-size: 36px; font-weight: 700; letter-spacing: -0.04em; }
	.ticker .price .small { font-size: 18px; color: #8a8a93; font-weight: 500; vertical-align: super; }

	.stats {
		display: grid; grid-template-columns: 1fr 1fr 1fr;
		border-top: 1px solid #25252b; border-bottom: 1px solid #25252b;
		margin: 6px 18px 0;
		font-family: 'JetBrains Mono', ui-monospace, monospace;
	}
	.stat { padding: 12px 0; text-align: center; }
	.stat + .stat { border-left: 1px solid #25252b; }
	.stat .v { font-size: 14px; font-weight: 700; color: #fff; }
	.stat .k { font-size: 9px; color: #8a8a93; letter-spacing: 0.1em; margin-top: 2px; }

	.buy-btn {
		display: block; width: calc(100% - 36px); margin: 16px 18px 0;
		padding: 18px; background: #00ff88; color: #0a0a0c;
	...
	.buy-btn:hover:not(:disabled) { background: #00cc6e; }
	.pay-row {
	...
	.thanks {
		margin: 16px 18px 0;
		padding: 18px; border: 1px solid #00ff88;
